<template>
    ...
    <div data-te-modal-init
        class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
        id="create_modal" tabindex="-1" aria-labelledby="edit_modalLabel" aria-hidden="true">    
        <div data-te-modal-dialog-ref
...
export default {
    ...
    methods: {
        ...
        async deleteItem() {
            let response = await deleteApiData({ url: `/api/contacts/` + this.deleteId, token: this.getToken() });
            if (response.success) {
