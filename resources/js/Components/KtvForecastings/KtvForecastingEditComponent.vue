...
export default {
    ...
    methods: {
        ...
        async getRoomForecastDetail() {
            let response = await getApiData({ url: `/api/forecasts_monthly_menu/${this.ktvForecastingId}`, token: this.getToken() });
            if (response.data) {
        ...
        async createRoomForcasting(){
            ...
            let url = `/api/forecasts_monthly_menu/${this.ktvForecastingId}`;
            let response = await postApiData({url: url, form_data: formData, token: this.getToken()});
            if(response.success){
